...
class Gemini {
  ...
  async multimodal(base64Image) {
    ...
    const prompt =
      "Extract the text from the attached image and summarize the key information in Thai. Ensure the summary includes an engaging and relevant title. The summary should be concise, consisting of no more than 2 to 3 paragraphs, and should be clear." // กำหนดค่า prompt
  ...
  async chat(cacheChatHistory, prompt) {
    ...
    const chatHistory = [
      ...
      },
      {
        ...
        parts: [
          {
            text: "สวัสดีนะ เราชื่อ Panya Chat Bot เราเป็นผู้จัดงานผู้ช่วยสรุปเนื้อหาของคุณ 😁",
          },
        ], // ข้อความเริ่มต้นจากโมเดล
      },
    ]
